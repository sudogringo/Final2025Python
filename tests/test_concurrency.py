...
class TestConcurrentStockOperations:
    ...
    def test_concurrent_order_detail_creation_prevents_overselling(self, db_session: Session, engine: Engine):
        """
    ...
    def test_concurrent_order_detail_updates_maintain_stock_consistency(self, db_session: Session, engine: Engine):
        category = CategoryModel(name="Electronics")
        db_session.add(category)
        db_session.commit()
        ...
        db_session.add_all([product, client, bill])
        db_session.commit()
        order = OrderModel(date=date(2025, 11, 17), delivery_method=DeliveryMethod.DRIVE_THRU, status=Status.PENDING, client_id=client.id_key, bill_id=bill.id_key)
        db_session.add(order)
        db_session.commit()
    ...
    def test_concurrent_order_detail_deletes_restore_stock_correctly(self, db_session: Session, engine: Engine):
        category = CategoryModel(name="Electronics")
        initial_stock = 100
        ...
        db_session.add_all([category, product, client, bill])
        db_session.commit()
        order = OrderModel(date=date(2025, 11, 17), delivery_method=DeliveryMethod.DRIVE_THRU, status=Status.PENDING, client_id=client.id_key, bill_id=bill.id_key)
        db_session.add(order)
        db_session.commit()
